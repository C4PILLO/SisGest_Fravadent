...
public class ProvinciaServiceImpl implements ProvinciaService {
	...
	public List<ProvinciaDTO> findAll() {
		return repositorio.findAll().stream().map(e -> modelMapper.map(e, ProvinciaDTO.class))
				.collect(Collectors.toList());
	}

	@Override
	public List<ProvinciaDTO> findAllCustom() {
		return repositorio.findAllCustom().stream().map(e -> modelMapper.map(e, ProvinciaDTO.class))
				.collect(Collectors.toList());
	}
