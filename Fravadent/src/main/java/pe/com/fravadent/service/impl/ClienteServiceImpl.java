...
public class ClienteServiceImpl implements ClienteService {
    ...
    public List<ClienteDTO> findAll() {
        return repositorio.findAll().stream().map(e -> modelMapper.map(e, ClienteDTO.class)).collect(Collectors.toList());
    }

    @Override
    public List<ClienteDTO> findAllCustom() {
        return repositorio.findAllCustom().stream().map(e -> modelMapper.map(e, ClienteDTO.class)).collect(Collectors.toList());
    }
