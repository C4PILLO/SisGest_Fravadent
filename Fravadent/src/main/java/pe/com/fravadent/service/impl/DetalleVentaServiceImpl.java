...
public class DetalleVentaServiceImpl implements DetalleVentaService {
	...
	public List<DetalleVentaDTO> findAll() {
		return repositorio.findAll().stream().map(e -> modelMapper.map(e, DetalleVentaDTO.class))
				.collect(Collectors.toList());
	}

	@Override
	public List<DetalleVentaDTO> findAllCustom() {
		return repositorio.findAllCustom().stream().map(e -> modelMapper.map(e, DetalleVentaDTO.class))
				.collect(Collectors.toList());
	}
