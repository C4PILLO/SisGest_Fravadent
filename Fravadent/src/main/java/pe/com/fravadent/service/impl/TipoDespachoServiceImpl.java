...
public class TipoDespachoServiceImpl implements TipoDespachoService {
	...
	public List<TipoDespachoDTO> findAll() {
		return repositorio.findAll().stream().map(e -> modelMapper.map(e, TipoDespachoDTO.class))
				.collect(Collectors.toList());
	}

	@Override
	public List<TipoDespachoDTO> findAllCustom() {
		return repositorio.findAllCustom().stream().map(e -> modelMapper.map(e, TipoDespachoDTO.class))
				.collect(Collectors.toList());
	}
