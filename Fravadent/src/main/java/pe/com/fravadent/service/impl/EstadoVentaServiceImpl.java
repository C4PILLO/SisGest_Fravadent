...
public class EstadoVentaServiceImpl implements EstadoVentaService {
	...
	public List<EstadoVentaDTO> findAll() {
		return repositorio.findAll().stream().map(e -> modelMapper.map(e, EstadoVentaDTO.class))
				.collect(Collectors.toList());
	}

	@Override
	public List<EstadoVentaDTO> findAllCustom() {
		return repositorio.findAllCustom().stream().map(e -> modelMapper.map(e, EstadoVentaDTO.class))
				.collect(Collectors.toList());
	}
