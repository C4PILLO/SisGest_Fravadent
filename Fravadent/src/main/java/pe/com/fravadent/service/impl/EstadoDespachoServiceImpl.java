...
public class EstadoDespachoServiceImpl implements EstadoDespachoService {
    ...
    public List<EstadoDespachoDTO> findAll() {
        return repositorio.findAll().stream().map(e -> modelMapper.map(e, EstadoDespachoDTO.class)).collect(Collectors.toList());
    }

    @Override
    public List<EstadoDespachoDTO> findAllCustom() {
        return repositorio.findAllCustom().stream().map(e -> modelMapper.map(e, EstadoDespachoDTO.class)).collect(Collectors.toList());
    }
