...
public class TipoMovimientoServiceImpl implements TipoMovimientoService {
	...
	public List<TipoMovimientoDTO> findAll() {
		return repositorio.findAll().stream().map(e -> modelMapper.map(e, TipoMovimientoDTO.class))
				.collect(Collectors.toList());
	}

	@Override
	public List<TipoMovimientoDTO> findAllCustom() {
		return repositorio.findAllCustom().stream().map(e -> modelMapper.map(e, TipoMovimientoDTO.class))
				.collect(Collectors.toList());
	}
